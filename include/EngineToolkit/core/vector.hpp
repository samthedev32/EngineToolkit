...
namespace EngineToolkit
{
  ...
  template <uint8_t D = 3, typename T = float>
  struct vec
  {
  ...
  template <uint8_t D, typename T>
  void vec<D, T>::operator=(const T &v)
  {
    for (uint8_t i = 0; i < D; i++)
      this->data[i] = v;
  }
  ...
  template <uint8_t D, typename T>
  T vec<D, T>::operator[](uint8_t i) const
  {
    return this->data[i % D];
  }

  template <uint8_t D, typename T>
  T &vec<D, T>::operator[](uint8_t i)
  {
    return this->data[i % D];
  }
  ...
  template <uint8_t D, typename T>
  vec<3> vec<D, T>::cross(vec<3> a, vec<3> b)
  {
