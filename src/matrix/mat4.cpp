...
mat4 mat4::operator*(const mat4 m) {
  ...
  for (int c = 0; c < 4; c++)
    for (int r = 0; r < 4; r++)
      matrix.data[r][c] =
          this->data[r][0] * m.data[0][c] + this->data[r][1] * m.data[1][c] +
          this->data[r][2] * m.data[2][c] + this->data[r][3] * m.data[3][c];
  return matrix;
}

vec3 mat4::operator*(const vec3 v) {
  return vec3(v.x * this->data[0][0] + v.y * this->data[1][0] +
...
mat4 mat4::perspective(float fovrads, float aspect, float fnear, float ffar) {
...
mat4 mat4::ortho(float left, float right, float bottom, float top, float fnear,
                 float ffar) {
  mat4 result = identity();
